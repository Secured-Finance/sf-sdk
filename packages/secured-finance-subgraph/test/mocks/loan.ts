...
export function createRegisterLoanEvent(
    ...
    event.parameters.push(
        new ethereum.EventParam('dealId', ethereum.Value.fromBytes(dealId))
    );

    return event;
}

export function createLiquidateLoanEvent(dealId: Bytes): Liquidate {
    let mockEvent = newMockEvent();
...
export function createLoanMarkToMarketEvent(
    ...
    event.parameters.push(
        new ethereum.EventParam(
            'prevPV',
            ethereum.Value.fromUnsignedBigInt(prevPV)
        )
    );
    event.parameters.push(
        new ethereum.EventParam(
            'currPV',
            ethereum.Value.fromUnsignedBigInt(currPV)
        )
    );
...
export function createLoanNovationEvent(
    ...
    event.parameters.push(
        new ethereum.EventParam(
            'currLender',
            ethereum.Value.fromAddress(currLender)
        )
    );
