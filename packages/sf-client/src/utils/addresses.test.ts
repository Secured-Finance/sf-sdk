...
import { getCollateralVaultAddressByCcy, getLendingMarketAddressByCcyAndTerm, packAddresses } from './addresses';
...
describe('Check addresses utils functions', function () {
    ...
    it('Try to pack addresses, check result hash', async () => {
        ...
        let expectedResult = '0xb656c9d359dd75ec87bbb7408823138e9a91aa5cb666a27a80210256a2372825';
    ...
    it('Try get lending market addresses by currency and term, check addresses', async () => {
        let result = getLendingMarketAddressByCcyAndTerm('FIL', '3 month', localhostNetworkID);
        let expectedResult = '0xdf12940a13c08715ab481e97cb521c3b395be019';
        assert.equal(result, expectedResult);

        result = getLendingMarketAddressByCcyAndTerm('ETH', '3 month', ropstenNetworkID);
        assert.equal(result, DEFAULT_ADDRESS);

        assert.throws(() => {
            getLendingMarketAddressByCcyAndTerm('BTC', '1 year', mainnetNetworkID);
        });
    });
});
