...
import { BilateralNetting, CollateralNetting, UnsettledCollateral } from "../../utils";

export const UnsettledPositionMock: React.FC<MockComponentProps> = ({ query, variables }) => {
    ...
    return (
        data.collateralAggregator
        ?
        data.collateralAggregator.collateralPositions.map((item: UnsettledCollateral, index: number) => (
            <div key={item.id}>
...
export const BilateralNettingMock: React.FC<MockComponentProps> = ({ query, variables }) => {
    const { loading, error, data } = useQuery(query, { variables: variables });
    ...
    return (
        data.collateralAggregator 
        ?
        data.collateralAggregator.bilateralPositions.map((item: BilateralNetting) => (
            <div key={item.id}>
